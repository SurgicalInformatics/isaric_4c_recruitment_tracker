...
```{r render_setup, include = FALSE, echo = FALSE}
library(rmarkdown)
library(tidyverse)
library(ggsci)
library(finalfit)
...
topline = topline %>% 
  ...
  mutate(place_name = ifelse(grepl('Nottingham University Hospitals NHS Trust', place_name) & grepl('Medical Centre Campus', place_name), 'Queen’s Medical Centre Campus', place_name))
```
...
### Patients in CCP-UK (at time of last update)
...
topline_by_country = topline %>% 
  filter(!is.na(country)) %>%
  filter(dsstdat >= '2020-03-01' & dsstdat <= today() + 1) %>% 
  group_by(dsstdat) %>% 
  summarise(n = n()) %>% 
  arrange(dsstdat) %>% 
...
gauge(round(count(topline %>% filter(demographics_complete == 'Complete') %>% 
  ...
  success = c(75, 100), warning = c(40, 74.99), danger = c(0, 39)
))
...
### Outcomes / Complications complete (4 week lag)
...
This dashboard was created by the [SurgicalInformatics Group](https://surgicalinformatics.org/) at the [University of Edinburgh](https://ed.ac.uk/) as part of the ISARIC-4C initiative. Please report any bugs or suggestions for improvement to us!
...
### Chart: Patients in CCP-UK (at time of last update)
...
### Chart: Patients in CCP-UK by Tier (at time of last update)
